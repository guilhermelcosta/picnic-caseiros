...
@section('title', trans('admin.contatos-tipo.actions.index'))

@section('body')

    <contatos-tipo-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/contatos-tipos') }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.contatos-tipo.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/contatos-tipos/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.contatos-tipo.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <!-- <th is='sortable' :column="'id'">{{ trans('admin.contatos-tipo.columns.id') }}</th> -->
                                        <th is='sortable' :column="'descricao'">{{ trans('admin.contatos-tipo.columns.descricao') }}</th>
                                    ...
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                            <span class="align-middle font-weight-light text-dark">@{{ clickedBulkItemsCount }} {{ trans('admin-ui.listing.selected_items') }}. <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/contatos-tipos')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-ui.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                                        href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">{{ trans('admin-ui.listing.uncheck_all_items') }}</a> </span>

                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/contatos-tipos/bulk-destroy')">{{ trans('admin-ui.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/contatos-tipos/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.contatos-tipo.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
