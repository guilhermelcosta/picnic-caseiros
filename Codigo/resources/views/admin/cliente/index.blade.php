                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'sobrenome'">{{ trans('admin.cliente.columns.sobrenome') }}</th>
                                        <th is='sortable' :column="'apelido'">{{ trans('admin.cliente.columns.apelido') }}</th>
                                        <th is='sortable' :column="'documento'">{{ trans('admin.cliente.columns.documento') }}</th>
                                        <th is='sortable' :column="'documentos_tipo_id'">{{ trans('admin.cliente.columns.documentos_tipo_id') }}</th>
                                        <th is='sortable' :column="'data_aniversario'">{{ trans('admin.cliente.columns.data_aniversario') }}</th>
                                        <th is='sortable' :column="'endereco_id'">{{ trans('admin.cliente.columns.endereco_id') }}</th>
                                        <th is='sortable' :column="'observacao_id'">{{ trans('admin.cliente.columns.observacao_id') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="11">
                                            <span class="align-middle font-weight-light text-dark">@{{ clickedBulkItemsCount }} {{ trans('admin-ui.listing.selected_items') }}.<a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/clientes')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-ui.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                                        href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">{{ trans('admin-ui.listing.uncheck_all_items') }}</a></span>

                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/clientes/bulk-destroy')">{{ trans('admin-ui.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                     ...
                                     <!-- <td>@{{ item.id }}</td> -->
                                        ...
                                        <td>@{{ formatDocument(item.documentos_tipo_id) }}</td>
                                        <td>@{{ item.data_aniversario | date }}</td>
