...
@section('title', trans('admin.insumo.custo.actions.index', ['insumo' => $insumo->descricao]))
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.insumo.custo.actions.index', ['insumo' => $insumo->descricao]) }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/insumos-custos/create') }}?insumo_id={{ $insumo->id }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.insumo.custo.actions.create', ['insumo' => $insumo->descricao]) }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tbody>
                                    ...
                                    <td>@{{ item.id }}</td>
                                        ...
                                        <td>@{{ item.insumo?.descricao ?? "" }}</td>
                                        <td>@{{ item.marca?.nome ?? "" }}</td>
                                        <td>@{{ item.fornecedor?.nome ?? "" }}</td>
                                        <td>@{{ item.data_compra | date }}</td>
                                        <td>@{{ formatMeasure(item.quantidade, item.unidade) }}</td>
                                        <!-- <td>@{{ item.unidade }}</td> -->
                                        <td>@{{ formatReal(item.valor_compra) }}</td>
                                        <td>@{{ item.valor_unitario }}</td>
                                        <td>@{{ item.is_atual ? 'Sim' : 'Não' }}</td>

                                        <td>
                                            <div class="row no-gutters">
                                                <div class="col-auto">
                                                    <a class="btn btn-sm btn-spinner btn-info" href="{{ url('admin/insumos-custos/create') }}?insumo_id={{ $insumo->id }}" :href="item.resource_url + '/edit?insumo_id=' + item.insumo.id" title="{{ trans('admin-ui.btn.edit') }}" role="button"><i class="fa fa-edit"></i></a>
                                                </div>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/insumos-custos/create') }}?insumo_id={{ $insumo->id }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.insumo.custo.actions.create', ['insumo' => $insumo->descricao]) }}</a>
                            </div>
                        </div>
                    </div>
                </div>
