...
@section('title', trans('admin.fornecedor.actions.index'))

@section('body')

    <fornecedor-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/fornecedores') }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.fornecedor.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/fornecedores/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.fornecedor.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <!-- <th is='sortable' :column="'id'">{{ trans('admin.fornecedor.columns.id') }}</th> -->
                                        <th is='sortable' :column="'nome'">{{ trans('admin.fornecedor.columns.nome') }}</th>
                                        <th is='sortable' :column="'endereco_id'">{{ trans('admin.fornecedor.columns.endereco_id') }}</th>
                                        <th is='sortable' :column="'nome_contato'">{{ trans('admin.fornecedor.columns.nome_contato') }}</th>
                                    ...
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="8">
                                            <span class="align-middle font-weight-light text-dark">@{{ clickedBulkItemsCount }} {{ trans('admin-ui.listing.selected_items') }}.<a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/fornecedores')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin-ui.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                                        href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">{{ trans('admin-ui.listing.uncheck_all_items') }}</a> </span>

                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/fornecedores/bulk-destroy')">{{ trans('admin-ui.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/fornecedores/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.fornecedor.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
